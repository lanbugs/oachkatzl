...
- **Survey Nodes** — pause a workflow and show a dynamically generated form in the UI, built at run time from a JSON schema (`string`, `int`, `bool`, `enum`, `multiselect`, `secret`, `textarea` fields) uploaded by the preceding task. Supports conditional logic — fields can be shown/hidden (`visible_if`) or have their dropdown options change (`options_by`) based on other answers. Submitted answers are written back to the artifact cache as JSON for the next node to consume.
